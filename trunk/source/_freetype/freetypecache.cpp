...
static FT_Error faceRequester(FTC_FaceID face_id, FT_Library library, FT_Pointer request_data, FT_Face* aface) {
	...
	error = FT_New_Face(library, face->path.c_str() , face->index, aface);
...
FreeTypeCache::FreeTypeCache( _u32 cacheBytesSize) {
...
void FreeTypeCache::loadSbit(FTC_ScalerRec scaler, _u32 charMapIndex, _u32 codepoint, FTC_SBit * sbit) const{
	...
	_u32 index = FTC_CMapCache_Lookup(_charMapCache, scaler.face_id, charMapIndex, codepoint);
