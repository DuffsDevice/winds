...
#include "_file/png.h"  // For the future, these should be merged.
#include "_file/pngenc.h"
...
int YsPngHuffmanTreeManager::MakeCodeLengthArray(unsigned int hLength[]) const
{
	...
	if(nullptr==root->zero && nullptr==root->one)
	{
		hLength[root->dat]=1;
		return YSOK;
	}
...
{
	if(nullptr==node->zero && nullptr==node->one)
	{
		hLength[node->dat]=depth;
	}
	else
	{
		if(nullptr!=node->zero)
		{
...
YsLazyMatchHashTable::YsLazyMatchHashTable(int hashSize,int allocSize)
{
	...
	nodeBuffer[allocSize-1].next=nullptr;
...
void YsLazyMatchHashTable::DeleteOldest(void)
{
	...
	for(i=0; i<hashSize; i++)
	{
		...
		for(ptr=hashTable[i]; nullptr!=ptr; ptr=ptr->next)
		{
			if(0!=first)
			{
				minIndex=ptr->index;
...
int YsPngCompressor::MakeLengthLiteral(int &hLit,unsigned int hLenLit[],int nCode,unsigned int codeArray[]) const
{
	...
	litTreeManager.MakeTreeFromData(nCode,codeArray,0,nullptr,285);  // zLib code(literal?) could be up to 285.
