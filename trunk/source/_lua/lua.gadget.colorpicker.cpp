...
_lua_colorpicker::_lua_colorpicker( lua_State* L )
{
	...
	this->setGadget( new _colorpicker( luaL_checkint( L , 1 ) , luaL_checkint( L , 2 ) , luaL_checkint( L , 3 ) , luaL_checkint( L , 4 ) , luaL_optint( L , 5 , COLOR_BLUE ) , luaL_optstyle( L , 6 ) ) );
	
	_lua_interface_input::input = (_colorpicker*)_lua_gadget::gadget;
}
...
Lunar<_lua_colorpicker>::PropertyType _lua_colorpicker::properties[] = {
	...
	{ "color" , &_lua_colorpicker::getIntValue , &_lua_colorpicker::setIntValue },
	LUA_CLASS_ATTR_END
};