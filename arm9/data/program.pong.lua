...
local ballVX = 1
local ballVY = 1
local pLength = 16 -- width of the paddle
local gameHeight = 60
local gameWidth = 100
local timer
local time
...
function main()
	...
	-- create the image
	pongBmp = ImageGadget( 0 , 0 , Bitmap( gameWidth , gameHeight ) , "noTransparentParts" )
...
function refresher( event )
	...
	-- Draw Paddles
	port.drawFilledRect( 1 , math.floor( p1 + 1 ) , 3 , pLength , "lime" )
	port.drawFilledRect( gameWidth - 4 , math.floor( p2 + 1 ) , 3 , pLength , "lime" )
	
	-- Draw middle Line and Ball
	port.drawVerticalDottedLine( math.floor( gameWidth/2 ) , 0 , gameHeight , "lime" )
	port.drawFilledCircle( math.floor( ballX ) , math.floor( ballY ) , 2 , "fuchsia" )
	
	-- Paint Scores
	local font = System.getFont("ArialBlack13")
	port.drawString( math.floor( gameWidth / 2 - 15 ) , 3 , font , scoreUser , "lime" , 10 )
	port.drawString( math.floor( gameWidth / 2 + 10 ) , 3 , font , scoreComputer , "lime" , 10 )
