...
int runNdsFile(const char* filename, int argc, const char** argv) 
{
	...
	return runNds (data_load_bin, data_load_bin_size, st.st_ino, false, true, argc, argv);
}
...
int installBootStub() {
	...
	memcpy(fake_heap_end,data_bootstub_bin,data_bootstub_bin_size);
	memcpy(fake_heap_end+data_bootstub_bin_size,data_load_bin,data_load_bin_size);

	int ret = dldiPatchLoader((data_t*)(fake_heap_end+data_bootstub_bin_size), data_load_bin_size,false);
	...
	bootcode->bootsize = data_load_bin_size;
