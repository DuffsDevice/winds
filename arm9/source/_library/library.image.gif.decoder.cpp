...
static gif_result gif_next_code(gif_animation *gif, int code_size);
...
static gif_result firstcode, oldcode;
...
*/		gif_data += 3;
		...
		*/
		if ((gif->frame_image = calloc(gif->width * gif->height, 4)) == NULL) {
			gif_finalise(gif);
			return GIF_INSUFFICIENT_MEMORY;
		}
...
static gif_result gif_initialise_sprite(gif_animation *gif, unsigned int width, unsigned int height) {
	...
	*/
	if ((buffer = calloc(max_width * max_height, 4)) == NULL)
		return GIF_INSUFFICIENT_MEMORY;
...
*/
ITCM_CODE gif_result gif_decode_frame(gif_animation *gif, unsigned int frame){
	...
	*/
	...
	/*	Get the frame data
	*/
	frame_data = (unsigned int*) gif->frame_image;
	if (!frame_data)
		return GIF_INSUFFICIENT_MEMORY;
	...
	if (!clear_image) {
		...
		*/
		for (y = 0; y < height; y++) {
			...
			*/
			x = width;
			bool respect_transparency = gif->frames[frame].transparency;
			unsigned char transparent_index = gif->frames[frame].transparency_index;
			while (x > 0) {
				...
				if (burst_bytes > 0) {
					...
					while (burst_bytes-- > 0) {
						...
						if (!respect_transparency || colour != transparent_index )
...
void gif_init_LZW(gif_animation *gif) {
	...
	stack_pointer = stack;
	do {
		firstcode = gif_next_code(gif, code_size);
...
static bool gif_next_LZW(gif_animation *gif) {
	gif_result code, incode, new_code;
	int block_size;
	...
	if (code < 0) {
		...
		block_size = 0;
		unsigned char* startData = &gif->gif_data[gif->buffer_position];
		unsigned char* tempData = startData;
		while (block_size != 1) {
			block_size = *tempData + 1;
			*tempData += block_size;
		}
		gif->current_error = GIF_FRAME_DATA_ERROR;
		return false;
	...
	if( code >= clear_code )
	{
		do
		{
			stack_pointer[0] = table[1][code];
			new_code = (gif_result) table[0][code];
			if(new_code < clear_code){
				code = new_code;
				stack_pointer++;
				break;
			}
			stack_pointer[1] = table[1][new_code];
			stack_pointer+=2;
			code = (gif_result) table[0][new_code];
			if(code != new_code)
				continue;
			gif->current_error = GIF_FRAME_DATA_ERROR;
			return false;
	...
	firstcode = (gif_result) table[1][code];
	*stack_pointer++ = table[1][code];

	if ((code = (gif_result) max_code) < (1 << GIF_MAX_LZW)) {
